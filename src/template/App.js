...
const App = () => {
  ...
  //ログイン状態の判定
  const onJudgmentAuthState = () => {
    auth.onAuthStateChanged((user) => {
      if (user) {
        const uid = user.uid;
        db.ref("users/" + uid + "/userName").once("value", (data) => {
          dispatch({ type: "SET_USERS_NAME", useName: data.val() });
        });
        return;
      } else {
        // ログイン処理
  ...
  useEffect(getRatedPopularDayWeek, [state.users.userName]);
  useEffect(onJudgmentAuthState, []);
  useEffect(() => {
